...
class AudioSwresample: FrameChange {
    ...
    func change(avframe: UnsafeMutablePointer<AVFrame>) throws -> MEFrame {
        ...
        // NuvioTV patch: swr_get_out_samples can return a negative AVERROR when
        // the resampler is in a bad state for an unusual audio frame. Feeding a
        // negative sample count into av_samples_get_buffer_size yields a
        // negative/huge dataSize, and allocating that aborts the whole process
        // (swift_abortAllocationFailure in AudioFrame.init). Drop the frame
        // instead of crashing.
        guard outSamples > 0 else {
            throw NSError(errorCode: .auidoSwrInit, userInfo: ["outSamples": Int(outSamples)])
        }
        ...
        // NuvioTV patch: same guard on the buffer size before it becomes an
        // allocation request.
        guard bufferSize[0] > 0 else {
            throw NSError(errorCode: .auidoSwrInit, userInfo: ["bufferSize": Int(bufferSize[0])])
        }
